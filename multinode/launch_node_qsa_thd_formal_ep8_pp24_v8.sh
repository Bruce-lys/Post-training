#!/usr/bin/env bash
# Four-node QSA THD formal launcher v8 (v7 + lengths file in swift shuffled dataset order). Validation-only unless explicitly confirmed.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_212k_qsa_thd_formal_ep8_pp24_v8.yaml}"
...
assert _lengths_doc['shuffle']['n'] == _rows, (_lengths_doc['shuffle']['n'], _rows)
...
print('[validate] QSA=true TP=8 PP=2 CP=1 EP=8 ETP=1 split=24/24 cpu_offload=off ce=native cache_release=on expandable_segments=on(PYTORCH_CUDA_ALLOC_CONF) resume=ckpt250 dp_balance_order=on(shuffled-order lengths) lengths_cache_rows=' + str(len(_lengths_doc['lengths'])))
