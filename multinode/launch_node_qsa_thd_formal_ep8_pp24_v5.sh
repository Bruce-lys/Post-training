#!/usr/bin/env bash
# Four-node QSA THD formal launcher v5 (v3 + cuda_cache_release plugin + resume from checkpoint-250; native CE). Validation-only unless explicitly confirmed.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_212k_qsa_thd_formal_ep8_pp24_v5.yaml}"
...
print('[validate] QSA=true TP=8 PP=2 CP=1 EP=8 ETP=1 split=24/24 cpu_offload=off ce=native cache_release=on resume=ckpt250 dp_balance_order=on lengths_cache_rows=' + str(len(_lengths_doc['lengths'])))
