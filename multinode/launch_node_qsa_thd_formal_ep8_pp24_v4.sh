#!/usr/bin/env bash
# Four-node QSA THD formal launcher v4 (v3 + TE cross-entropy + resume from checkpoint-250). Validation-only unless explicitly confirmed.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_212k_qsa_thd_formal_ep8_pp24_v4.yaml}"
...
assert config['cross_entropy_loss_fusion'] is True and config['cross_entropy_fusion_impl'] == 'te'
assert config['megatron_extra_kwargs']['cross_entropy_fusion_impl'] == 'te', 'TE CE must be passed via megatron_extra_kwargs'
...
print('[validate] QSA=true TP=8 PP=2 CP=1 EP=8 ETP=1 split=24/24 cpu_offload=off ce=te resume=ckpt250 dp_balance_order=on lengths_cache_rows=' + str(len(_lengths_doc['lengths'])))
