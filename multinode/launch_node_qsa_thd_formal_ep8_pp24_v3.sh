#!/usr/bin/env bash
# Four-node QSA THD formal launcher v3 (EP=8, PP split 24/24, no optimizer CPU offload, dp_balance_order + lengths cache). Validation-only unless explicitly confirmed.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_212k_qsa_thd_formal_ep8_pp24_v3.yaml}"
...
print('[validate] QSA=true TP=8 PP=2 CP=1 EP=8 ETP=1 split=24/24 cpu_offload=off dp_balance_order=on lengths_cache_rows=' + str(len(_lengths_doc['lengths'])))
