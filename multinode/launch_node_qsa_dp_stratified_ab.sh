#!/usr/bin/env bash
# Four-node length-stratified DP scheduling experiment. Defaults to validation only.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_qsa_dp_stratified_ab.yaml}"
...
dataset = '/kwkj-k8s/llm_team/lys/megatron-swift/multinode/smoke_data/dp_stratified_ab/real_stratified_ab_72rows.jsonl'
assert config['dataset'] == dataset
assert sum(1 for _ in open(dataset, encoding='utf-8')) == 72
summary = json.load(open(dataset + '.summary.json'))
assert summary['steps'] == 9
assert summary['step_labels'] == ['W', 'AS', 'BS', 'BM', 'AM', 'AX1', 'BX1', 'BX2', 'AX2']
assert summary['mixed_sample_counts'] == {'short': 6, 'medium': 8, 'long': 2}
for name, pair in summary['groups'].items():
    assert pair['A']['total_tokens'] == pair['B']['total_tokens'], name
    assert pair['B']['dp_token_gap'] <= pair['A']['dp_token_gap'], name
...
print(f'[validate] dataset={dataset} rows=72 steps=9')
for name, pair in summary['groups'].items():
    print(f'[validate] {name}: A_gap={pair["A"]["dp_token_gap"]} B_gap={pair["B"]["dp_token_gap"]} total={pair["A"]["total_tokens"]}')
print(f'[validate] mcore_bridge={source}')
print(f'[validate] qsa_core={inspect.getfile(QSASparseCoreAttention)}')
print('[validate] QSA=true packing=false padding_free=true CP=1 PP=2 TP=8 EP=16 train_iters=9 nsys=off')
PY
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[launch] DRY_RUN=1: validation complete; stratified DP A/B smoke was not started." | tee -a "$LOG"
  exit 0
fi
