#!/usr/bin/env bash
# Four-node QSA THD formal-opt launcher. Validation-only unless explicitly confirmed.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_212k_qsa_thd_overlay_formal_opt.yaml}"
...
expected_output = '/kwkj-k8s/llm_team/lys/megatron-swift/outputs/tb226_flash_next_lora_4node_212k_qsa_thd_overlay_formal_opt'
...
print('[validate] QSA=true TP=8 PP=2 CP=1 EP=16 ETP=1 split=22/26 dp_token_balance=on')
