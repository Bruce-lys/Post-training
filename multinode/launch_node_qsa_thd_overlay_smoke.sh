#!/usr/bin/env bash
# Four-node QSA THD smoke launcher. It defaults to validation-only mode.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_qsa_top24_thd_overlay_smoke.yaml}"
...
expected_dataset = '/kwkj-k8s/llm_team/lys/megatron-swift/multinode/smoke_data/qsa_top24_212k.jsonl'
assert config['dataset'] == expected_dataset, 'launcher is restricted to the top-24 smoke dataset'
...
assert 1 <= int(config['train_iters']) <= 3
...
print('[validate] packing=false padding_free=true group_by_length=false CP=1 SP=true max_length=212992 train_iters='
      f"{config['train_iters']}")
PY
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[launch] DRY_RUN=1: validation complete; training was not started." | tee -a "$LOG"
  exit 0
fi
