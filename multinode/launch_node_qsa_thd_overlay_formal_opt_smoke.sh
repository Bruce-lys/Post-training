#!/usr/bin/env bash
# Four-node smoke for formal_opt DP token-balance plugin. Defaults to validation only.
set -euo pipefail
...
CONFIG="${CONFIG:-$MULTINODE/tb226_flash_next_lora_4node_qsa_thd_overlay_formal_opt_smoke.yaml}"
NODES="${NODES:?set NODES=\"host0 host1 host2 host3\" (host0 = master)}"
MASTER_PORT="${MASTER_PORT:-29632}"
DRY_RUN="${DRY_RUN:-1}"
START_OPT_SMOKE="${START_OPT_SMOKE:-NO}"
TAG="${TAG:-tb226_flash_next_lora_4node_qsa_formal_opt_smoke}"
TS="${TS:-formalopt_smoke_$(date +%Y%m%d_%H%M%S)}"
...
assert str(config["ENV"]["DP_TOKEN_BALANCE"]) == "True"
...
assert any(path.endswith("dp_token_balance.py") for path in config["external_plugins"])
...
plugin_path = Path("/kwkj-k8s/llm_team/lys/megatron-swift/multinode/plugins/dp_token_balance.py")
spec = importlib.util.spec_from_file_location("dp_token_balance", plugin_path)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
module.self_check()
print(f"[validate] dataset={dataset} rows=40")
print(f"[validate] mcore_bridge={source}")
print(f"[validate] qsa_core={inspect.getfile(QSASparseCoreAttention)}")
print("[validate] QSA=true packing=false padding_free=true TP=8 PP=2 CP=1 EP=16 split=22/26")
print("[validate] dataset_shuffle=true dp_token_balance=on train_iters=5 nsys=off save=off")
PY
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[launch] DRY_RUN=1: validation complete; opt smoke was not started." | tee -a "$LOG"
  exit 0
fi
[[ "$DRY_RUN" == "0" ]] || { echo "ABORT: DRY_RUN must be 0 or 1"; exit 1; }
[[ "$START_OPT_SMOKE" == "YES_I_CONFIRM" ]] || {
  echo "ABORT: set START_OPT_SMOKE=YES_I_CONFIRM on all four nodes"
  exit 1
}
