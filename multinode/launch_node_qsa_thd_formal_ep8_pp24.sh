#!/usr/bin/env bash
# Four-node QSA THD formal launcher v2 (EP=8, PP split 24/24, no optimizer CPU offload). Validation-only unless explicitly confirmed.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_212k_qsa_thd_formal_ep8_pp24.yaml}"
...
expected_output = '/kwkj-k8s/llm_team/lys/megatron-swift/outputs/tb226_flash_next_lora_4node_212k_qsa_thd_formal_ep8_pp24'
...
assert str(config['ENV'].get('DP_TOKEN_BALANCE')) == 'True'
...
plugin_path = plugin_dir / 'dp_token_balance.py'
...
print('[validate] QSA=true TP=8 PP=2 CP=1 EP=8 ETP=1 split=24/24 cpu_offload=off dp_token_balance=on')
