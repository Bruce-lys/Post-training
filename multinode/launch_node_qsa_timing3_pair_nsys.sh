#!/usr/bin/env bash
# Four-node, three-step full-iteration timing launcher. Defaults to validation only.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_qsa_timing3_nsys.yaml}"
NODES="${NODES:?set NODES=\"host0 host1 host2 host3\" (host0 = master)}"
MASTER_PORT="${MASTER_PORT:-29623}"
DRY_RUN="${DRY_RUN:-1}"
START_PROFILE_SMOKE="${START_PROFILE_SMOKE:-NO}"
CAPTURE_STEP="${CAPTURE_STEP:-3}"
NSYS_CAPTURE_RANGE_END="${NSYS_CAPTURE_RANGE_END:-}"
NSYS_NODES="${NSYS_NODES:-hd03-gpu2-0019 hd03-gpu2-0005}"
TAG="${TAG:-$(basename "$CONFIG" .yaml)}"
TS="${TS:-$(date +%Y%m%d_%H%M%S)}"

read -r -a NODE_ARR <<<"$NODES"
export NNODES=${#NODE_ARR[@]}
[[ "$NNODES" -eq 4 ]] || { echo "ABORT: profile smoke requires exactly four nodes"; exit 1; }
...
PROFILE_DIR=$ROOT/multinode/profiles/$TAG/$TS
...
[[ "$CAPTURE_STEP" == "1" || "$CAPTURE_STEP" == "3" ]] || {
  echo "ABORT: CAPTURE_STEP must be 1 or 3"
  exit 1
}
if [[ -z "$NSYS_CAPTURE_RANGE_END" ]]; then
  if [[ "$CAPTURE_STEP" == "3" ]]; then
    NSYS_CAPTURE_RANGE_END=none
  else
    NSYS_CAPTURE_RANGE_END=stop
  fi
fi
[[ "$NSYS_CAPTURE_RANGE_END" == "none" || "$NSYS_CAPTURE_RANGE_END" == "stop" ]] || {
  echo "ABORT: NSYS_CAPTURE_RANGE_END must be none or stop"
  exit 1
}
export NSYS_CAPTURE_STEP="$CAPTURE_STEP"
...
expected_dataset = '/kwkj-k8s/llm_team/lys/megatron-swift/multinode/smoke_data/profile_timing3/real_step5_repeated_24rows.jsonl'
assert config['dataset'] == expected_dataset
assert sum(1 for _ in open(expected_dataset, encoding='utf-8')) == 24
summary = json.load(open(expected_dataset + '.summary.json'))
assert summary['steps'] == 3 and summary['samples_per_step'] == 8
assert summary['tokens_per_step'] == 827160
assert config['ENV']['QSA_SPARSE_KERNEL'] == 'True'
assert int(os.environ['NSYS_CAPTURE_STEP']) in (1, 3)
assert config['packing'] is False and config['padding_free'] is True
assert config['context_parallel_size'] == 1 and config['sequence_parallel'] is True
...
assert any('nsys_iteration_capture.py' in p for p in config['external_plugins'])
...
print(f'[validate] dataset={expected_dataset} rows=24 tokens_per_step={summary["tokens_per_step"]}')
print(f'[validate] mcore_bridge={source}')
print(f'[validate] qsa_core={inspect.getfile(QSASparseCoreAttention)}')
print('[validate] QSA=true packing=false padding_free=true '
      f'train_iters=3 capture_step={os.environ["NSYS_CAPTURE_STEP"]} final_save=disabled')
PY
...
echo "[launch] host=$ME rank=$NODE_RANK/$NNODES master=$MASTER_ADDR:$MASTER_PORT dry_run=$DRY_RUN nsys=$PROFILE_THIS_NODE" | tee -a "$LOG"
echo "[launch] config=$CONFIG overlay=$actual_commit" | tee -a "$LOG"

if [[ "$DRY_RUN" == "1" ]]; then
  echo "[launch] DRY_RUN=1: validation complete; timing smoke was not started." | tee -a "$LOG"
  exit 0
fi
[[ "$DRY_RUN" == "0" ]] || { echo "ABORT: DRY_RUN must be 0 or 1"; exit 1; }
[[ "$START_PROFILE_SMOKE" == "YES_I_CONFIRM" ]] || {
  echo "ABORT: set START_PROFILE_SMOKE=YES_I_CONFIRM on all four nodes"
  exit 1
}
...
if [[ "$PROFILE_THIS_NODE" == "1" ]]; then
  ...
  LOCAL_NSYS="/tmp/${TAG}_${TS}_node${NODE_RANK}"
  echo "[nsys] local_report=${LOCAL_NSYS}.nsys-rep capture_step=$NSYS_CAPTURE_STEP capture_range_end=$NSYS_CAPTURE_RANGE_END" | tee -a "$LOG"
  set +e
  nsys profile \
  ...
  if [[ -f "$report" ]]; then
    ...
    ls -lh "$report" "$PROFILE_DIR/nsys.node${NODE_RANK}.nsys-rep" | tee -a "$LOG"
  else
