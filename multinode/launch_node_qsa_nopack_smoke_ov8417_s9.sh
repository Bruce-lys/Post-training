...
EXPECTED_OVERLAY_COMMIT=8417fcf510ada1b88b1052aaa2581638f628fe3a   # qsa-bitmap-chunked (bit-exact chunked selection_to_block_bitmap)
CONFIG="${CONFIG:-$MULTINODE/tb226_flash_next_lora_4node_qsa_nopack_ep8_pp24_s9_shuffledlen_smoke.yaml}"
...
_ldoc = json.loads(lengths_file.read_text())
import numpy as _np
from swift.utils.np_utils import get_seed as _get_seed
assert _ldoc.get("shuffle", {}).get("hf_seed") == int(_get_seed(_np.random.RandomState(int(config.get("data_seed", 42))))), "lengths file must be in swift shuffled order for this data_seed"
