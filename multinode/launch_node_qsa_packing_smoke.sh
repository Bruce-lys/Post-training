#!/usr/bin/env bash
# Four-node packing smoke launcher (main smoke + A/B equivalence runs). Validation-only unless confirmed.
# Usage on EACH of the four nodes (host0 = master), same TS on all nodes:
#   NODES="h0 h1 h2 h3" TS=<shared tag> DRY_RUN=1 bash launch_node_qsa_packing_smoke.sh
#   NODES="h0 h1 h2 h3" TS=<shared tag> DRY_RUN=0 START_PACKING_SMOKE=YES_I_CONFIRM bash launch_node_qsa_packing_smoke.sh
# Pick the config with CONFIG=... (default: main smoke). DRY_RUN=1 hides all GPUs from the validator.
set -euo pipefail
...
CONFIG="${CONFIG:-$MULTINODE/tb226_flash_next_lora_4node_qsa_packing_smoke.yaml}"
NODES="${NODES:?set NODES=\"host0 host1 host2 host3\" (host0 = master)}"
MASTER_PORT="${MASTER_PORT:-29640}"
DRY_RUN="${DRY_RUN:-1}"
START_PACKING_SMOKE="${START_PACKING_SMOKE:-NO}"
TAG="${TAG:-tb226_flash_next_lora_4node_qsa_packing_smoke}"
...
# Validation must never touch a GPU (the nodes may be running formal training).
...
is_ab = "_ab_" in name
is_single = "_ab_single_" in name
unpacked = name.endswith("ab_unpacked") or name.endswith("ab_single_unpacked")
smoke_dir = Path("/kwkj-k8s/llm_team/lys/megatron-swift/multinode/smoke_data/packing_smoke")
summary = json.loads((smoke_dir / "summary.json").read_text())
expected = summary["ab_single"] if is_single else (summary["ab"] if is_ab else summary["subset"])
...
if unpacked:
    ...
    assert config["global_batch_size"] == expected["unpacked_gbs"]
else:
    assert config["packing"] is True and config["packing_length"] == 212992
    if is_ab:
        assert config["global_batch_size"] == expected["packed_gbs"]
    else:
...
assert config["expert_model_parallel_size"] in (8, 16)  # 8 = expert all-to-all stays inside one node
assert config["expert_tensor_parallel_size"] == 1
assert config["sequence_parallel"] is True
assert config["decoder_first_pipeline_num_layers"] == 22
...
assert config["train_iters"] == (1 if is_ab else 8)
...
if is_ab:
    ...
    assert config["dataset_shuffle"] is True
names = [Path(p).name for p in config["external_plugins"]]
assert not any("dp_token_balance" in n for n in names)
assert "packed_ple_varlen.py" in names and "ple_chunked.py" in names and "mem_probe.py" in names
assert names.index("packed_ple_varlen.py") < names.index("ple_chunked.py")
assert not any("nsys" in n.lower() or "pp_split_timing" in n for n in names)
...
# packed_ple_varlen.py raises at import if the overlay PLE source no longer matches its patch.
for plugin in config["external_plugins"]:
    if plugin.endswith("packed_ple_varlen.py"):
        spec = importlib.util.spec_from_file_location("packed_ple_varlen", plugin)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
print(f"[validate] config={name} dataset={dataset} rows={expected['rows']} sha256=ok")
print(f"[validate] mcore_bridge={source}")
print(f"[validate] qsa_core={inspect.getfile(QSASparseCoreAttention)}")
print(f"[validate] packing={config['packing']} padding_free=true gbs={config['global_batch_size']} train_iters={config['train_iters']}")
print(f"[validate] QSA=true TP=8 PP=2 CP=1 EP={config['expert_model_parallel_size']} ETP=1 split=22/26 plugins=packed_ple_varlen>ple_chunked>mem_probe save=off")
PY
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[launch] DRY_RUN=1: validation complete; packing smoke was not started." | tee -a "$LOG"
  exit 0
fi
[[ "$DRY_RUN" == "0" ]] || { echo "ABORT: DRY_RUN must be 0 or 1"; exit 1; }
[[ "$START_PACKING_SMOKE" == "YES_I_CONFIRM" ]] || {
  echo "ABORT: set START_PACKING_SMOKE=YES_I_CONFIRM on all four nodes"
  exit 1
}
