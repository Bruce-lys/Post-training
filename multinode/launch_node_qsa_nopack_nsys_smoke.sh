#!/usr/bin/env bash
# Four-node no-packing smoke launcher WITH Nsight Systems capture of one steady-state step.
# nsys runs only on NSYS_NODES (default: node rank 0 = PP stage 0, node rank 2 = PP stage 1); the capture window is
# gated by cudaProfilerApi from plugins/nsys_iteration_capture.py at step NSYS_CAPTURE_STEP. Validation-only unless confirmed.
...
CONFIG="${CONFIG:-$MULTINODE/tb226_flash_next_lora_4node_qsa_nopack_ep8_pp24_nsys_smoke.yaml}"
...
TAG="${TAG:-tb226_flash_next_lora_4node_qsa_nopack_nsys_smoke}"
...
is_baseline = "baseline24" in name
expected = summary["baseline24"] if is_baseline else summary["subset"]
...
assert config["dataset_shuffle"] is (not is_baseline)
assert config["train_iters"] == (3 if is_baseline else 8)
...
assert "nsys_iteration_capture.py" in names
assert not any("packed_ple_varlen" in n or "pp_split_timing" in n for n in names)
import os
assert 1 <= int(os.environ["NSYS_CAPTURE_STEP"]) <= config["train_iters"]
...
print(f"[validate] packing=false padding_free=true gbs=8 train_iters=8 EP={config['expert_model_parallel_size']} split={config['decoder_first_pipeline_num_layers']}/{48 - config['decoder_first_pipeline_num_layers']}")
print("[validate] QSA=true TP=8 PP=2 CP=1 ETP=1 plugins=" + ">".join(n[:-3] for n in names) + " save=off nsys_capture_step=" + os.environ["NSYS_CAPTURE_STEP"])
PY
...
echo "[launch] nsys_this_node=$PROFILE_THIS_NODE nsys_nodes=[$NSYS_NODES] capture_step=$NSYS_CAPTURE_STEP" | tee -a "$LOG"
