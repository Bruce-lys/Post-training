#!/usr/bin/env bash
# Four-node W-A-B-B-A DP load-balancing experiment. Defaults to validation only.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_qsa_dp_balance_ab.yaml}"
NODES="${NODES:?set NODES=\"host0 host1 host2 host3\" (host0 = master)}"
MASTER_PORT="${MASTER_PORT:-29625}"
DRY_RUN="${DRY_RUN:-1}"
START_PERF_SMOKE="${START_PERF_SMOKE:-NO}"
TAG="${TAG:-$(basename "$CONFIG" .yaml)}"
TS="${TS:-$(date +%Y%m%d_%H%M%S)}"

read -r -a NODE_ARR <<<"$NODES"
export NNODES=${#NODE_ARR[@]}
[[ "$NNODES" -eq 4 ]] || { echo "ABORT: experiment requires exactly four nodes"; exit 1; }
...
dataset = '/kwkj-k8s/llm_team/lys/megatron-swift/multinode/smoke_data/dp_balance_ab/real_step5_wabba_40rows.jsonl'
assert config['dataset'] == dataset
assert sum(1 for _ in open(dataset, encoding='utf-8')) == 40
summary = json.load(open(dataset + '.summary.json'))
assert summary['steps'] == 5 and summary['step_sequence'] == ['W', 'A', 'B', 'B', 'A']
assert summary['A']['tokens_per_step'] == summary['B']['tokens_per_step'] == 827160
assert summary['A']['dp_token_totals'] == [318474, 508686]
assert sorted(summary['B']['dp_token_totals']) == [413389, 413771]
...
assert any('iteration_timing.py' in path for path in config['external_plugins'])
...
print(f'[validate] dataset={dataset} rows=40 sequence=W-A-B-B-A tokens_per_step=827160')
print(f'[validate] A_dp_tokens={summary["A"]["dp_token_totals"]} gap={summary["A"]["dp_token_gap"]}')
print(f'[validate] B_dp_tokens={summary["B"]["dp_token_totals"]} gap={summary["B"]["dp_token_gap"]}')
print(f'[validate] mcore_bridge={source}')
print(f'[validate] qsa_core={inspect.getfile(QSASparseCoreAttention)}')
print('[validate] QSA=true packing=false padding_free=true CP=1 PP=2 TP=8 EP=16 train_iters=5 nsys=off')
PY
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[launch] DRY_RUN=1: validation complete; DP A/B smoke was not started." | tee -a "$LOG"
  exit 0
fi
[[ "$DRY_RUN" == "0" ]] || { echo "ABORT: DRY_RUN must be 0 or 1"; exit 1; }
[[ "$START_PERF_SMOKE" == "YES_I_CONFIRM" ]] || {
  echo "ABORT: set START_PERF_SMOKE=YES_I_CONFIRM on all four nodes"
  exit 1
}
