#!/usr/bin/env bash
# Four-node QSA THD formal launcher v6 (v5 + PYTORCH_CUDA_ALLOC_CONF so expandable segments are really on). Validation-only unless explicitly confirmed.
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/multinode/tb226_flash_next_lora_4node_212k_qsa_thd_formal_ep8_pp24_v6.yaml}"
